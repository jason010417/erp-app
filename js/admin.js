...
function renderAdminSuppliers(q = ''){
  ...
  el.innerHTML = list.map(s => {
    ...
    const isConsignment = extra.consignmentType === 'consignment';
    const hasStory = !!extra.story;
    return `
    <div class="admin-item" onclick="openSupplierEditor('${s.id}')">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div class="admin-item-sub" style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
          <span>${s.id}</span>
          ${isConsignment ? `<span class="status-badge badge-pending" style="font-size:10px;">寄賣</span>` : ''}
          ${hasStory ? `<span style="font-size:10px;color:var(--green);">📖 有故事</span>` : ''}
        </div>
      </div>
